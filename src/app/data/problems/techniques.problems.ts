...
export const TECHNIQUE_PROBLEMS: WorkedProblem[] = [
  // -------------------------------------------------------------- two pointers
  {
    ...
    optimal: {
      ...
      code: `Arrays.sort(a);
for (int i = 0; i < n - 2; i++) {
    if (i > 0 && a[i] == a[i - 1]) continue;          // duplicate anchor
    int lo = i + 1, hi = n - 1;
    while (lo < hi) {
...
while (mid <= high) {
  ...
  },
  {
    ...
    optimal: {
      ...
      complexity: 'O(n) time, O(1) space',
      language: 'java',
      code: `int lo = 0, hi = n - 1, leftMax = 0, rightMax = 0, water = 0;
while (lo < hi) {
    if (h[lo] < h[hi]) {
  ...
  // ------------------------------------------------------------ sliding window
  {
    ...
    optimal: {
      ...
      language: 'java',
      code: `Map<Character, Integer> need = countOf(t);
int required = need.size(), formed = 0, left = 0, bestLen = MAX;
...
return best == Integer.MAX_VALUE ? 0 : best;`,
  ...
  // -------------------------------------------------------------- binary search
  {
    ...
    optimal: {
      ...
      language: 'java',
      code: `int lower(int[] a, int target) {          // first index with a[i] >= target
...
int first = lower(a, target);
int last  = lower(a, target + 1) - 1;
...
return -1;`,
  ...
  },
  {
    ...
    optimal: {
      ...
      language: 'java',
      code: `int lo = max(weights), hi = sum(weights);
while (lo < hi) {
    int mid = lo + (hi - lo) / 2;
...
int daysNeeded(int[] w, int cap) {
  ...
  },
  {
    ...
    optimal: {
      ...
      language: 'java',
      code: `// search i = how many elements come from 'a' on the left side
int lo = 0, hi = n;
while (lo <= hi) {
    int i = (lo + hi) / 2, j = (n + m + 1) / 2 - i;

    int aLeft  = i == 0 ? MIN : a[i - 1], aRight = i == n ? MAX : a[i];
    int bLeft  = j == 0 ? MIN : b[j - 1], bRight = j == m ? MAX : b[j];

    if (aLeft <= bRight && bLeft <= aRight) return median(aLeft, bLeft, aRight, bRight);
  ...
  },
  {
    ...
    optimal: {
      ...
      language: 'java',
      code: `Arrays.sort(intervals, (x, y) -> x[0] - y[0]);
...
for (int[] interval : intervals) {
    ...
    else merged.add(interval.clone());
}`,
    },
  ...
  },
  {
    ...
    optimal: {
      ...
      language: 'java',
      code: `while (i < mid && j < hi) {
    if (a[i] <= a[j]) buffer[k++] = a[i++];
    else {
        count += mid - i;          // all remaining left elements are greater
        buffer[k++] = a[j++];
    }
}`,
    },
  ...
  },
  {
    ...
    optimal: {
      ...
      language: 'java',
      code: `String[] parts = Arrays.stream(a).mapToObj(String::valueOf).toArray(String[]::new);
Arrays.sort(parts, (x, y) -> (y + x).compareTo(x + y));

if (parts[0].equals("0")) return "0";        // all zeroes
